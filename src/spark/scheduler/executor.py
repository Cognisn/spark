...
class ActionExecutor:
    ...
    def execute(self, action_id: int) -> None:
        ...
        try:
            ...
            logger.info("Executing action: '%s' (model: %s)", action["name"], action.get("model_id"))
            ...
            try:
                ...
                logger.info("Action '%s' completed (%d in / %d out tokens)",
                            action["name"], result.get("input_tokens", 0), result.get("output_tokens", 0))
                ...
                if new_count >= max_failures:
                    ...
                    logger.warning(
                        "Action '%s' disabled after %d consecutive failures", action["name"], new_count
                    )
    ...
    def _run_action(self, db: Any, action: dict) -> dict[str, Any]:
        ...
        if not llm:
            raise RuntimeError(f"Could not initialise LLM for model '{model_id}'. Check provider is enabled and API key is configured.")
        ...
        return {"content": "Max tool iterations reached.", "input_tokens": total_input, "output_tokens": total_output}
    ...
    def _init_llm(self, model_id: str) -> Any:
        ...
        logger.error("No provider could initialise model '%s'. Enabled providers: %s",
                     model_id,
    ...
    def _init_ollama(self, settings: Any) -> Any:
        ...
        return OllamaProvider(base_url=settings.get("providers.ollama.base_url", "http://localhost:11434"))
